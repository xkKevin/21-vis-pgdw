...
export default {
  ...
  methods: {
    getUpload(e){
      ...
      generateGlyphs('upload_morpheus_generate_transform_specs', 'morpheusData');
    ...
    initData() {
      ...
      // this.selectCase();
      sendVue_gp(this);
      record_log("morpheus", returnCitySN);
    },
    initEditor() {
      // 初始化编辑器，确保dom已经渲染
    ...
    getMorpheus(case_string = "") {
      ...
      .then((response) => {
        if(response.data.error_info){
          ...
          this.detectChanges();
          generateGlyphs('morpheus_generate_transform_specs', 'morpheusData');
          this.gm_flag_count -= 1
        }
      })
      .catch((error) => {
