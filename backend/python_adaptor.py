...
\n'''.format(space_index=space_index, value=match_r[0][1],line_num=line_num,pandas_abbr=pandas_abbr) 
    ...
    with open(script_exec_name, "w", encoding='utf-8') as fp:
        codes += '''{space_index}with open("colnames.txt","w",encoding='utf-8') as fp:
{space_index}    json.dump(col_states,fp,indent=2)'''.format(space_index=space_index)
        fp.write(codes)
    ...
    with open("colnames.txt","r",encoding='utf-8') as fp:
...
def generate_transform_specs(script_name):
    ...
    parser_info, col_states, pandas_abbr = execScript(script_name)
